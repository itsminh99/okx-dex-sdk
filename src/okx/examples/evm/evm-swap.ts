...
async function main() {
  try {
    ...
    if (args.length !== 3) {
      console.log(
        "Usage: ts-node xlayer-swap.ts <amount> <fromTokenAddress> <toTokenAddress>"
      );
      console.log("\nExamples:");
      console.log("  # Swap 0.1 X Layer to USDC");
      console.log(
        `  ts-node xlayer-swap.ts 0.1 ${TOKENS.NATIVE} ${TOKENS.USDC}`
      );
      process.exit(1);
    }
    ...
    const quoteAmount = (parseFloat(amount) * Math.pow(10, DECIMALS)).toFixed(
      0
    );
    ...
    if (result.details) {
      console.log("\nDetails:");
      console.log(
        `Input: ${result.details.fromToken.amount} ${result.details.fromToken.symbol}`
      );
      console.log(
        `Output: ${result.details.toToken.amount} ${result.details.toToken.symbol}`
      );
