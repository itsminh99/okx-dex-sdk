...
async function main() {
  try {
    ...
    console.log(
      "- From Token Amount:",
      swapData.data[0].routerResult.fromTokenAmount
    );
    console.log(
      "- To Token Amount:",
      swapData.data[0].routerResult.toTokenAmount
    );
    console.log(
      "- Price Impact:",
      swapData.data[0].routerResult.priceImpactPercentage
    );
    ...
    console.log(
      "- Explorer URL:",
      `https://web3.okx.com/explorer/base/tx/${txHash}`
    );
    console.log("\n📋 To track this transaction:");
    console.log(
      `   npx ts-node src/okx/examples/solana/evm-order-tracking.ts track ${orderId} ${walletAddress} 8453`
    );
  } catch (error) {
